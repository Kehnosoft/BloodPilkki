...
func _ready():
	...
	self.connect("area_entered", Callable(self, "_on_attack_area_entered"))
	self.connect("area_exited", Callable(self, "_on_attack_area_exit"))
...
func remove_available_attack_target(target):
	...
	for i in range(0, len(targets)):
		if targets[i] == target:
			get_parent()._available_attack_targets.remove_at(i)
			var target_indicator = target.get_node("AttackIndicator")
			if target_indicator:
